...
To get input from the webcam openCV is used. The most difficult aspect of of getting input was differentiaing between different colors in the images. To help with this the images are preprocessed by being blurred and converted to the HSV color space. HSV is used over RGB as it only has one channel dedicated to color making it easier to adjust for different colors. However, different lighting condtions reflect different HSV values for each color. To get the most accurate values for HSV data was collected for four different lighting conditions.
